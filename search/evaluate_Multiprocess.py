...
def process_task(i, data_list, arguments, output_list):
    ...
    url = data_list[i]['image']
    # image = Image.open(os.path.join(arguments.image_prefix, url))
    image = os.path.join(arguments.image_prefix, url)
    ...
    Task = MCTS_Task(data, arguments.propose_method, arguments.value_method, arguments.branch, arguments.end_gate,
                        ...
                        low=arguments.low, high=arguments.high, evaluate=arguments.evaluate, answer=answer, lang='en', image=image)
...
def parse_args():
    ...
    base_args.add_argument('--image_prefix', type=str, default='/cpfs/29f69eb5e2e60f26/code/sft_intern/lh/slz/math_mm/Data/MathVista/')
