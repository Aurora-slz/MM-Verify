...
def get_inference_model_llavaOneVision(model_dir):
    # pipe = pipeline("image-to-text", model=model_dir)
...
#     return split_response
def get_local_response_llama(query, model=None, tokenizer=None, max_length=2048, truncation=True, max_new_tokens=2048, temperature=0.7, do_sample=False, image=None, pipe=None, processor=None):
    ...
    while cnt:
        try:
            ...
            output = model.generate(**inputs, max_new_tokens=1024, do_sample=True, pad_token_id=tokenizer.eos_token_id, temperature=1.2)
            # print(processor2.decode(output[0][2:], skip_special_tokens=True))
            all_response = processor.decode(output[0][2:], skip_special_tokens=True).split('assistant\n')[-1].strip()
            # print('** all_response: ', all_response)
            # exit(0)
            # all_response = response.replace('\nassistant\nLava', '').strip()
            break
        except Exception as e:
            print(f'Error:{e}, obtain response again...\n')
            cnt -= 1
...
import base64
def get_local_response_api(query, tokenizer=None, max_length=2048, truncation=True, max_new_tokens=2048, temperature=0.7, do_sample=False, image=None, pipe=None, processor=None, client=None):
    cnt = 2
