...
import requests
import json
from models.inference_models import get_local_response, get_inference_model, get_inference_model_llama, get_local_response_llama, get_inference_model_mistral, get_local_response_mistral, get_inference_model_llavaOneVision, get_local_response_api
...
def local_inference_model(query, max_length=2048, truncation=True, do_sample=False, max_new_tokens=2048,
    ...
    if inference_type == 'glm':
        ...
        # return get_local_response_llama(query, model, tokenizer=inference_tokenizer, max_new_tokens=max_new_tokens,
        #                                 temperature=temperature, do_sample=do_sample, image=image, processor=processor)
        return get_local_response_api(query, max_new_tokens=max_new_tokens,
                                        temperature=temperature, do_sample=do_sample, image=image, client=client)
    else:
