...
def main():
    ...
    for i in tqdm(range(0, len(results))):
        prompt_list = []
        query = results[i]['conversations'][0]['value'].split('\nQuery:')[-1].strip()
        real_answer = results[i]['choices'][results[i]['answer_index']]
        qwq_answer_context = results[i]['conversations'][-1]['value'].strip()
        
        if(qwq_answer_context.find('**Final Answer**') != -1):
            qwq_answer_context = 'The final answer is ' + qwq_answer_context.split('**Final Answer**')[-1].strip()
        
        qwq_answer_context_prompt = create_test_prompt(demo_prompt, query, qwq_answer_context)
        mcts_tmp = tokenizer.apply_chat_template(
            [{'role': 'user', 'content': qwq_answer_context_prompt}],
            tokenize=False,
        )
        ...
        qwq_answer = outputs[0].outputs[0].text
        
            
        results[i]['answerExtract_qwq'] = qwq_answer.replace('<|start_header_id|>assistant<|end_header_id|>', '').strip()
        results[i]['answerExtract_real'] = real_answer
        
        
        qwq_answer = results[i]['answerExtract_qwq']
        real_answer = results[i]['answerExtract_real']
        

        if(qwq_answer == real_answer):
